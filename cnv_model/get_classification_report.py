...
def get_patient_precision_recall(hp):
    ...
    for i in range(folds_num):
        ...
        with (open(f"{hp['root_dir']}roc_out_{i}.p", "rb")) as openfile:
    ...
    plt.title("confusion matrix CNV model")
 
    plt.savefig(f"{hp['root_dir']}cm_CNV.eps",dpi=500)
    plt.savefig(f"{hp['root_dir']}cm_CNV.png",dpi=500)
...
hp = hyperparams()
get_patient_precision_recall(hp)